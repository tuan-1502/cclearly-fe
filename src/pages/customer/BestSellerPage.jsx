...
import { Glasses, Scan, Search, SlidersHorizontal, Sparkles, Star } from 'lucide-react'
...
const BestSellerPage = () => {
  ...
  const bestSellers = useMemo(() => {
    const getScore = (product) => (product.rating || 0) * 100 + (product.reviewCount || 0) + (product.isSale ? 50 : 0)
  ...
  const summaryStats = useMemo(() => {
    ...
    const avgRating = bestSellers.length
      ? (bestSellers.reduce((total, product) => total + (product.rating || 0), 0) / bestSellers.length).toFixed(1)
      : '0.0'

    return {
      saleCount,
      avgRating,
      frameCount: bestSellers.filter((product) => product.type === 'frame').length,
      lensCount: bestSellers.filter((product) => product.type === 'lens').length,
    }
  }, [bestSellers])
  ...
  const shownFrom = filteredProducts.length === 0 ? 0 : (normalizedCurrentPage - 1) * pageSize + 1
  const shownTo = Math.min(normalizedCurrentPage * pageSize, filteredProducts.length)
    ...
    <div className="min-h-screen bg-[#f5f8ff]">
      <section className="border-b border-[#dbe4f4] bg-white">
        <div className="mx-auto max-w-[1180px] px-4 py-8">
          ...
          <h1 className="mt-4 text-3xl font-bold uppercase tracking-tight text-[#1d2433]">Best Sellers</h1>
