...
const getMenuItems = (role) => {
  const items = {
    ...
    [ROLES.OPERATIONS]: [
      ...
      { path: "/operations/board", label: "Điều phối đơn", icon: ClipboardList },
      { path: "/operations/orders", label: "Xử lý đơn hàng", icon: Package },
      { path: "/operations/lens-lab", label: "Gia công tròng", icon: TestTube },
      { path: "/operations/shipping", label: "Giao vận & Tracking", icon: Truck },
    ...
    [ROLES.MANAGER]: [
      ...
      { path: "/manager/orders", label: "Quản lý đơn hàng", icon: ShoppingCart },
...
export const AdminLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-[#ececec] overflow-hidden">
      {/* SIDEBAR */}
      <aside
        className={`bg-white shadow-[0_10px_30px_rgba(13,22,39,0.06)] transition-all duration-300 flex flex-col h-full ${sidebarOpen ? "w-[260px]" : "w-[80px]"
          }`}
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#ececec] shrink-0">
          {sidebarOpen && (
            <Link to="/" className="flex items-center gap-2 group transition-opacity hover:opacity-80">
              <svg viewBox="0 0 120 40" className="h-7 w-12 text-[#1f7a5a]" fill="none" stroke="currentColor" strokeWidth="2.5">
